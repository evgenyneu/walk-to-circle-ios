...
class TegScrolledContent {
  ...
  class func layoutContentViewInScrollView(contentView: UIView, scrollView: UIView) {
    ...
    iiAutolayoutConstraints.fillParent(contentView, parentView: scrollView, margin: 0, vertically: false)

    iiAutolayoutConstraints.fillParent(contentView, parentView: scrollView, margin: 0, vertically: true)
    
    // Content view width is equal to scroll view width
    iiAutolayoutConstraints.equalWidth(contentView, viewTwo: scrollView, constraintContainer: scrollView)
  }
